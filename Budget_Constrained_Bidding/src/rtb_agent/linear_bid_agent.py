...
class LinearBidAgent():
    ...
    def __init__(self):
        ...
        self.budget_spend = 0
        self.cur_day = 1
        self.BETA = [0.92, 0.97, 0.99, 1, 1.01, 1.03, 1.08]
...
#        action = min(self.target_value * state['vid_len'] * 1e9,
#                        (self.budget - self.budget_spend) * 1e9)
        action = min(random.choice(self.BETA)*self.target_value * state['vid_len'],
